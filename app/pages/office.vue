<template>
  <div>
    <div class="mb-6">
      <h1 class="text-2xl font-bold text-white">Office</h1>
      <p class="text-gray-400 text-sm mt-0.5">Live view of your agents</p>
    </div>
    ...
    <div class="bg-gray-800 border border-gray-700 rounded-xl p-8 min-h-96 relative overflow-hidden">
      ...
      <div class="relative grid grid-cols-2 md:grid-cols-3 gap-6">
        ...
        >
          ...
          >
            <div v-if="member.status === 'busy'" class="space-y-1">
              <div class="h-1.5 bg-green-500/40 rounded animate-pulse" style="width: 80%" />
              <div class="h-1.5 bg-green-500/30 rounded animate-pulse" style="width: 60%" />
              <div class="h-1.5 bg-green-500/20 rounded animate-pulse" style="width: 70%" />
            </div>
            <div v-else class="flex items-center justify-center h-8">
              <span class="text-xs text-gray-600">idle</span>
            </div>
          </div>

          <!-- Name + status -->
          <p class="font-medium text-sm text-white">{{ member.name }}</p>
          <p class="text-xs text-gray-400 capitalize">{{ member.status }}</p>
        </div>
      </div>
    </div>
  </div>
