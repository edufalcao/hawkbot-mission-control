  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-gray-400 text-sm mt-0.5">Cron jobs & scheduled tasks</p>
      </div>
    ...
    <div v-if="pending" class="space-y-2">
      <div v-for="i in 6" :key="i" class="h-16 bg-gray-800 rounded-lg animate-pulse" />
    </div>
    ...
    <div v-else-if="events.length" class="space-y-2">
      ...
      >
        <div class="w-2 h-2 rounded-full flex-shrink-0" :class="statusColor(event.status)" />
        ...
        <UBadge :color="event.status === 'scheduled' ? 'green' : 'gray'" size="xs">
          {{ event.status }}
        </UBadge>
      </div>
    </div>
    ...
    <div v-else class="text-center py-16 text-gray-500">
      <UIcon name="i-lucide-calendar-off" class="w-12 h-12 mx-auto mb-3 opacity-40" />
